...
/**
 * Реализация DAO для управления задачами с коллекцией вложений типа Map.
 */
public class TaskComponentMapDAOImpl implements TaskComponentMapDAO {
    private static final Logger log = Logger.getLogger(TaskComponentMapDAOImpl.class);
    ...
    public List<Task> findAll() {
        ...
        try (EntityManager em = DatabaseConfig.getLab4EntityManager()) {
            log.info(String.format(Constants.LOG_DB_OPERATION, methodName));
            TypedQuery<Task> query = em.createQuery("SELECT t FROM ComponentMapTask t", Task.class);
