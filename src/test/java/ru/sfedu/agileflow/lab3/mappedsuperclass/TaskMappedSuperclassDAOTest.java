...
/**
 * Тестовый класс для проверки операций DAO с задачами (MappedSuperclass).
 */
public class TaskMappedSuperclassDAOTest {
    private static final Logger log = Logger.getLogger(TaskMappedSuperclassDAOTest.class);
    private TaskMappedSuperclassDAO taskDAO;
    ...
    public void setUp() {
        ...
        taskDAO = new TaskMappedSuperclassDAOImpl();
    ...
    public void tearDown() {
        ...
        try {
            ...
            for (Task task : tasks) {
                taskDAO.delete(task.getId(), task.getClass());
            }
    ...
    /**
     * Тестирование создания задачи типа Feature.
     * Тип: Позитивный
     */
    @Test
    public void testCreateFeatureTask() {
        String methodName = "testCreateFeatureTask";
        log.info(String.format("%s [1] Начало выполнения метода", methodName));
        try {
            FeatureTask task = new FeatureTask("New Feature", "Feature Description", TaskStatus.TO_DO, 2, null, null, "Must work");
            log.info("testCreateFeatureTask [1] Создание задачи типа Feature");
            taskDAO.create(task);
            assertNotNull("Идентификатор задачи должен быть установлен", task.getId());
            log.info("testCreateFeatureTask [2] Задача успешно создана с ID: " + task.getId());
            log.info(String.format("%s [2] Метод успешно завершен", methodName));
        } catch (Exception e) {
            log.error(String.format("%s [3] Произошла ошибка: %s", methodName, e.getMessage()), e);
            fail("Не удалось создать задачу: " + e.getMessage());
    ...
    /**
     * Тестирование поиска задачи типа Bug по идентификатору.
     * Тип: Позитивный
     */
    @Test
    public void testFindByIdBugTask() {
        String methodName = "testFindByIdBugTask";
        log.info(String.format("%s [1] Начало выполнения метода", methodName));
        try {
            BugTask task = new BugTask("Fix Bug", "Bug Description", TaskStatus.TO_DO, 1, null, null, "High");
            log.info("testFindByIdBugTask [1] Создание задачи типа Bug");
            taskDAO.create(task);
            Integer id = task.getId();

            log.info("testFindByIdBugTask [2] Поиск задачи по ID: " + id);
            Optional<BugTask> found = taskDAO.findById(id, BugTask.class);
            assertTrue("Задача должна быть найдена", found.isPresent());
            assertEquals("Заголовок задачи должен совпадать", "Fix Bug", found.get().getTitle());
            log.info("testFindByIdBugTask [3] Задача успешно найдена");
            log.info(String.format("%s [2] Метод успешно завершен", methodName));
        } catch (Exception e) {
            log.error(String.format("%s [3] Произошла ошибка: %s", methodName, e.getMessage()), e);
    ...
    /**
     * Тестирование поиска задачи типа Feature по идентификатору.
     * Тип: Позитивный
     */
    @Test
    public void testFindByIdFeatureTask() {
        String methodName = "testFindByIdFeatureTask";
        log.info(String.format("%s [1] Начало выполнения метода", methodName));
        try {
            FeatureTask task = new FeatureTask("New Feature", "Feature Description", TaskStatus.TO_DO, 2, null, null, "Must work");
            log.info("testFindByIdFeatureTask [1] Создание задачи типа Feature");
            taskDAO.create(task);
            Integer id = task.getId();

            log.info("testFindByIdFeatureTask [2] Поиск задачи по ID: " + id);
            Optional<FeatureTask> found = taskDAO.findById(id, FeatureTask.class);
            assertTrue("Задача должна быть найдена", found.isPresent());
            assertEquals("Заголовок задачи должен совпадать", "New Feature", found.get().getTitle());
            log.info("testFindByIdFeatureTask [3] Задача успешно найдена");
            log.info(String.format("%s [2] Метод успешно завершен", methodName));
        } catch (Exception e) {
            log.error(String.format("%s [3] Произошла ошибка: %s", methodName, e.getMessage()), e);
            fail("Не удалось найти задачу: " + e.getMessage());
    ...
    public void testFindByIdNotFound() {
        ...
        try {
            ...
            Optional<BugTask> found = taskDAO.findById(999, BugTask.class);
    ...
    /**
     * Тестирование обновления задачи типа Bug.
     * Тип: Позитивный
     */
    @Test
    public void testUpdateBugTask() {
        String methodName = "testUpdateBugTask";
        log.info(String.format("%s [1] Начало выполнения метода", methodName));
        try {
            BugTask task = new BugTask("Bug Task", "Description", TaskStatus.TO_DO, 1, null, null, "High");
            log.info("testUpdateBugTask [1] Создание задачи");
            taskDAO.create(task);
            Integer id = task.getId();

            task.setTitle("Updated Bug Task");
            task.setStatus(TaskStatus.IN_PROGRESS);
            log.info("testUpdateBugTask [2] Обновление задачи");
            taskDAO.update(task);

            log.info("testUpdateBugTask [3] Проверка обновленной задачи");
            Optional<BugTask> updated = taskDAO.findById(id, BugTask.class);
            ...
            log.info("testUpdateBugTask [4] Задача успешно обновлена");
            log.info(String.format("%s [2] Метод успешно завершен", methodName));
        } catch (Exception e) {
            log.error(String.format("%s [3] Произошла ошибка: %s", methodName, e.getMessage()), e);
    ...
    /**
     * Тестирование удаления задачи типа Feature.
     * Тип: Позитивный
     */
    @Test
    public void testDeleteFeatureTask() {
        String methodName = "testDeleteFeatureTask";
        log.info(String.format("%s [1] Начало выполнения метода", methodName));
        try {
            FeatureTask task = new FeatureTask("Feature Task", "Description", TaskStatus.TO_DO, 2, null, null, "Must work");
            log.info("testDeleteFeatureTask [1] Создание задачи");
            taskDAO.create(task);
            Integer id = task.getId();

            log.info("testDeleteFeatureTask [2] Удаление задачи с ID: " + id);
            taskDAO.delete(id, FeatureTask.class);

            log.info("testDeleteFeatureTask [3] Проверка удаления");
            Optional<FeatureTask> deleted = taskDAO.findById(id, FeatureTask.class);
            assertFalse("Задача не должна быть найдена", deleted.isPresent());
            log.info("testDeleteFeatureTask [4] Задача успешно удалена");
            log.info(String.format("%s [2] Метод успешно завершен", methodName));
        } catch (Exception e) {
            log.error(String.format("%s [3] Произошла ошибка: %s", methodName, e.getMessage()), e);
